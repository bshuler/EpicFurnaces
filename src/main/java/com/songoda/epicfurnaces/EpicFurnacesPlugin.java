...
public class EpicFurnacesPlugin extends JavaPlugin implements EpicFurnaces {
    ...
    public void onEnable() {
        // Compatibility is gated by plugin.yml's api-version rather than by
        // sniffing the server's NMS package name (obsolete on modern,
        // Mojang-mapped Paper servers - see CLAUDE.md).

        INSTANCE = this;
        console.sendMessage(Methods.formatText("&a============================="));
        console.sendMessage(Methods.formatText("&7EpicFurnaces " + this.getDescription().getVersion() + " by &5Brianna <3&7!"));
    ...
    private void setupRecipies() {
        ...
        if (getConfig().getBoolean("Main.Use Custom Recipes")) {
            ...
            for (String key : cs.getKeys(false)) {
                ...
                NamespacedKey recipeKey = new NamespacedKey(this, "furnace_upgrade_" + key.toLowerCase());
                getServer().addRecipe(new FurnaceRecipe(recipeKey, new ItemStack(result, amount), item, 0.0f, 200));
            }
        }
    }
