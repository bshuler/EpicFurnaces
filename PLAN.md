...
in `minecraft_plugins/PLAN.md`.

### Finding: `/EpicFurnaces` does not exist; `/epicfurnaces` does

The first run of this test FAILED here, and it was right to. `plugin.yml` declares the command as `EpicFurnaces:` (capitalised), and the live server answered `Unknown or incomplete command` to that spelling. Probing the running server by hand settled what was actually going on:

| typed at the console | result |
|---|---|
| `epicfurnaces` | prints the plugin's help — registered |
| `ef` (the declared alias) | prints the plugin's help — registered |
| `EpicFurnaces` | `Unknown or incomplete command` |

Bukkit registers a `plugin.yml` command under `label.toLowerCase()`, and Paper's Brigadier console dispatch is case-**sensitive**. So the capitalisation in this `plugin.yml` is cosmetic at best: nobody has ever been able to type `/EpicFurnaces`. The harness was fixed to send the lowercase label (helpers-version 3) rather than the declared spelling — comparing the declared spelling would report a false "never registered" for every capitalised declaration in the program. The `plugin.yml` itself is left alone: lowercasing it would change nothing a user can observe, and the capitalised key is what upstream shipped.
